...
public class PaddleScript : MonoBehaviour
{
    ...
    {
        // Assuming the size is in the x-axis of the local scale
        return spriteRend.size.x;
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "Ball")
        {
            ...
            float yComponent = BallsManagerScript.Instance.bmBallPaddleSpeed;

            Vector2 direction = new Vector2(xComponent, yComponent);
            direction.Normalize();
            float ballSpeed = BallsManagerScript.Instance.bmBallPaddleSpeed;
        ...
        // Log the speed
        //Debug.Log("Paddle Ball Speed: " + BallsManagerScript.Instance.bmBallPaddleSpeed);
    }
}
